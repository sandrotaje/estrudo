...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  const selectedCount =
    ...
    state.selectedCircleIds.length +
    state.selectedConstraintIds.length;
  ...
  const handleConstraintClick = (type: ConstraintType, needsInput: boolean) => {
    ...
    const isLineCircleDist =
      type === ConstraintType.DISTANCE &&
      lIds.length === 1 &&
      cIds.length === 1;
    const isCircleCircleDist =
      type === ConstraintType.DISTANCE && cIds.length === 2;
    const isLineCircleCoincident =
      type === ConstraintType.COINCIDENT &&
      lIds.length === 1 &&
      cIds.length === 1;
    const isPointLineCoincident =
      type === ConstraintType.COINCIDENT &&
      lIds.length === 1 &&
      pIds.length === 1;
    const isPointLineMidpoint =
    ...
    if (type === ConstraintType.ANGLE) {
      if (lIds.length !== 1 && lIds.length !== 2) {
        alert(
          "Select exactly one line (angle with horizontal) or two lines (angle between lines)"
        );
        return;
      }
    }
    ...
    if (type === ConstraintType.TANGENT) {
      ...
      const isCircleCircle =
        cIds.length === 2 && lIds.length === 0 && pIds.length === 0;
      const isPointCircle =
        pIds.length === 1 && cIds.length === 1 && lIds.length === 0;

      if (!isLineCircle && !isCircleCircle && !isPointCircle) {
        alert(
          "Select (1 Line + 1 Circle) OR (2 Circles) OR (1 Point + 1 Circle) to apply tangent"
        );
        return;
      }
    }
        ...
        if (lIds.length === 1 && cIds.length === 1) {
          ...
          if (line && circle) {
            ...
            if (l2 > 0) {
              const distToCenter =
                Math.abs(
                  dy * center.x - dx * center.y + p2.x * p1.y - p2.y * p1.x
                ) / Math.sqrt(l2);
      ...
      if (!isNaN(val)) {
        if (pendingConstraint.id) {
          ...
          onAddConstraint(
            ...
            pendingConstraint.pIds,
            pendingConstraint.lIds,
            pendingConstraint.cIds,
            val
          );
        }
      }
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
                  ...
                  <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    {editingFeatureId !== feature.id && (
                      <button
                        ...
                        className="px-2 py-1 bg-blue-600 text-white text-[8px] font-bold uppercase rounded hover:bg-blue-500"
                      >
                        Edit
                      </button>
                    )}
                    <button
                      ...
                      className="px-2 py-1 bg-red-600 text-white text-[8px] font-bold uppercase rounded hover:bg-red-500"
                    >
                      Del
                    </button>
                  </div>
            ...
            <div className="bg-[#222] p-4 rounded-lg border border-blue-500/30 animate-in fade-in slide-in-from-left-4">
              <h3 className="text-xs font-bold text-blue-400 uppercase mb-3">
                {pendingConstraint.id
                  ? "Edit Value"
                  : pendingConstraint.type === ConstraintType.ANGLE
                  ? "Set Angle (Deg)"
                  : pendingConstraint.type === ConstraintType.DISTANCE
                  ? "Set Distance"
                  : "Set Radius"}
              </h3>
              <form onSubmit={confirmInput}>
                ...
                <div className="flex gap-2">
                  <button
                    type="submit"
                    className="flex-1 bg-blue-600 hover:bg-blue-500 text-white py-2 rounded text-xs font-bold uppercase"
                  >
                    Apply
                  </button>
                  <button
              ...
              {state.constraints.map((c, i) => {
                ...
                return (
                  <div
                    ...
                    className={`p-2 text-[10px] rounded flex items-center justify-between cursor-pointer group ${
                      isSelected
          ...
          <section>
            <div className="flex items-center justify-between mb-2">
              <h2 className="text-[10px] font-bold text-gray-600 uppercase">
                Elements
              </h2>
              <div className="flex gap-2">
                {(state.selectedLineIds.length > 0 ||
                  ...
                  <button
                    ...
                    className="text-[10px] text-gray-400 hover:text-white font-bold uppercase transition-colors"
                  >
                    Toggle Const.
                  </button>
                )}
            ...
            <div className="space-y-1 bg-[#111] rounded-lg p-2 max-h-48 overflow-y-auto">
              ...
              {state.circles.map((c, i) => (
                <div
                  key={c.id}
                  className={`p-2 text-[10px] rounded ${
                    state.selectedCircleIds.includes(c.id)
                      ? "bg-blue-500/20 text-blue-300"
                      : "text-gray-400"
                  }`}
                >
                  ○ Circle {i + 1} (R: {Math.round(c.radius)}){" "}
              ...
              {state.arcs?.map((a, i) => (
                <div
                  key={a.id}
                  className={`p-2 text-[10px] rounded ${
                    state.selectedCircleIds.includes(a.id)
                      ? "bg-blue-500/20 text-blue-300"
                      : "text-gray-400"
                  }`}
                >
                  ⌒ Arc {i + 1} (R: {Math.round(a.radius)}){" "}
