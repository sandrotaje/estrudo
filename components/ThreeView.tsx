...
import { SketchState, Feature, Line, ConstraintType, Arc, Circle, Point } from '../types';
...
interface ThreeViewProps {
  ...
  onCommitExtrusion: (depth: number, operation: 'NEW' | 'CUT', throughAll: boolean, featureType: 'EXTRUDE' | 'REVOLVE', revolveAngle?: number, revolveAxisId?: string) => void;
  onUpdateFeatureParams?: (id: string, updates: Partial<Feature>) => void;
  onSketchOnFace?: (lines: Line[], points: Point[], transform: number[], arcs: Arc[], circles: Circle[]) => void;
  onClose: () => void;
...
const ThreeView: React.FC<ThreeViewProps> = ({ 
  ...
  const [featureType, setFeatureType] = useState<'EXTRUDE' | 'REVOLVE'>(initialFeatureParams?.featureType || 'EXTRUDE');
  const [localDepth, setLocalDepth] = useState(initialFeatureParams?.extrusionDepth || state.extrusionDepth || 50);
  const [revolveAngle, setRevolveAngle] = useState(initialFeatureParams?.revolveAngle || 360);
  const [operation, setOperation] = useState<'NEW' | 'CUT'>(initialFeatureParams?.operation || 'NEW');
  const [throughAll, setThroughAll] = useState(initialFeatureParams?.throughAll || false);
  const [isConfigOpen, setIsConfigOpen] = useState(!!initialFeatureParams);
  const [errorMsg, setErrorMsg] = useState<string | null>(null);
  
  // Axis Selection State
  const [activeAxisId, setActiveAxisId] = useState<string | null>(initialFeatureParams?.revolveAxisId || null);

  // Selection & Interaction State
  const [selectedSketchElements, setSelectedSketchElements] = useState<string[]>([]);
  const [hoveredSketchElement, setHoveredSketchElement] = useState<string | null>(null);
  ...
  const fitView = useCallback(() => {
    if (!cameraRef.current || !controlsRef.current || !historyGroupRef.current || !meshGroupRef.current) return;
    ...
    const offset = new THREE.Vector3().subVectors(cameraRef.current.position, center).normalize();
  ...
  const getShapesFromSketch = (sketchState: SketchState, allowedIds?: string[]) => {
    ...
    const getAdj = (id: string) => { if (!adj.has(id)) adj.set(id, []); return adj.get(id)!; };
    ...
    const lines = sketchState.lines.filter(l => !l.construction && l.id !== axisId && isAllowed(l.id));
    const arcs = (sketchState.arcs || []).filter(a => !a.construction && isAllowed(a.id));
    const circles = sketchState.circles.filter(c => !c.construction && isAllowed(c.id));

    lines.forEach(l => {
        const u = find(l.p1), v = find(l.p2);
        if(u !== v) { getAdj(u).push({ to: v, isArc: false }); getAdj(v).push({ to: u, isArc: false }); }
    });
    arcs.forEach(a => {
        const u = find(a.p1), v = find(a.p2);
        if(u !== v) { getAdj(u).push({ to: v, isArc: true, arcData: a }); getAdj(v).push({ to: u, isArc: true, arcData: a }); }
    });
    ...
    const cycles: { edges: { from: string, to: string, isArc: boolean, arcData?: any }[] }[] = [];
    const getCoords = (canonicalId: string) => {
        const p = sketchState.points.find(p => p.id === canonicalId) || sketchState.points.find(p => find(p.id) === canonicalId);
    ...
    for(const startNode of adj.keys()) {
        ...
        while(true) {
            ...
            for(const edge of neighbors) {
                if (edge.to !== prev) {
                    if (!visitedInPath.has(edge.to)) { nextEdge = edge; break; }
                    if (path.length > 2 && edge.to === path[0]) { nextEdge = edge; break; }
                }
            }
            if (!nextEdge) {
                 if (neighbors.some(e => e.to === path[0]) && path.length > 2 && prev !== path[0]) nextEdge = neighbors.find(e => e.to === path[0]);
                 else break;
            }
            if (nextEdge) {
                if (nextEdge.to === path[0]) {
                    ...
                    const cycleEdges = [];
                    for(let i=0; i<path.length; i++) cycleEdges.push({ from: path[i], to: edgePath[i].to, isArc: edgePath[i].isArc, arcData: edgePath[i].arcData });
                    cycles.push({ edges: cycleEdges });
                    path.forEach(id => visitedGlobal.add(id));
                    break;
                }
                if (visitedInPath.has(nextEdge.to)) break;
                path.push(nextEdge.to); edgePath.push(nextEdge); visitedInPath.add(nextEdge.to);
                prev = curr; curr = nextEdge.to;
    ...
    const shapeData = shapes.map(s => {
        ...
        return { shape: s, points, area: isNaN(area) ? 0 : area, parent: null as any };
    });
    ...
    for (let i = 0; i < validShapeData.length; i++) {
        ...
        for (let j = i - 1; j >= 0; j--) {
            ...
            if (current.points.length > 0 && isPointInside(current.points[0], potential.points)) {
                if (!bestParent || potential.area < bestParent.area) bestParent = potential;
            }
        }
  ...
  const generateExtrusionGeometry = (shapes: THREE.Shape[], depth: number) => {
    ...
    const extrudeSettings = { steps: 1, depth: depth, bevelEnabled: false, curveSegments: 32 };
    return new THREE.ExtrudeGeometry(shapes, extrudeSettings);
  };

  const generateRevolveGeometry = (shapes: THREE.Shape[], axisLineId: string | undefined, angleDeg: number, sketchState: SketchState) => {
    ...
    const axisVec = new THREE.Vector2().subVectors(axisEnd, axisStart).normalize();
    ...
    shapes.forEach(shape => {
         ...
         const quaternion = new THREE.Quaternion().setFromUnitVectors(up, targetAxis);
  ...
  const generateGeometryForFeature = (featType: 'EXTRUDE' | 'REVOLVE', shapeList: THREE.Shape[], depth: number, angle: number, axisId: string | undefined, sketch: SketchState) => {
      if (featType === 'EXTRUDE') {
    ...
    features.forEach((feature) => {
        ...
        if (!brush) {
            ...
            if (geom) {
                ...
                if (feature.transform) brush.applyMatrix4(new THREE.Matrix4().fromArray(feature.transform));
  ...
  useEffect(() => {
     ...
     // Only visualize if we are NOT previewing a solid operation
     ...
     state.lines.forEach(line => {
        ...
        if (p1 && p2) {
            ...
            mesh.userData = { sketchId: line.id, type: 'line', construction: line.construction };
            group.add(mesh);
        }
     });

     // Render Circles/Arcs
     const renderArc = (id: string, centerId: string, radius: number, p1Id?: string, p2Id?: string, construction?: boolean) => {
         ...
         const mesh = new THREE.Line(geom, mat);
         mesh.userData = { sketchId: id, type: 'curve', construction: construction };
         group.add(mesh);
     };

     state.circles.forEach(c => renderArc(c.id, c.center, c.radius, undefined, undefined, c.construction));
     state.arcs.forEach(a => renderArc(a.id, a.center, a.radius, a.p1, a.p2, a.construction));

  }, [state, isConfigOpen, currentTransform, selectedSketchElements]); // Hover state removed from here to prevent full rebuilds
  ...
  useEffect(() => {
     ...
     sketchVisGroupRef.current.children.forEach((obj) => {
         if (obj instanceof THREE.Line) {
             ...
             if (obj.material instanceof THREE.LineBasicMaterial || obj.material instanceof THREE.LineDashedMaterial) {
  ...
  const previewShapes = useMemo(() => {
      ...
      const shapes = getShapesFromSketch(state, selectedSketchElements.length > 0 ? selectedSketchElements : undefined);
      if (shapes.length === 0) {
      ...
      const geom = generateGeometryForFeature(featureType, shapes, depth, revolveAngle, activeAxisId || undefined, state);
      ...
      setErrorMsg(null);
      return geom;
  }, [state, localDepth, throughAll, operation, featureType, revolveAngle, activeAxisId, isConfigOpen, selectedSketchElements]);
  ...
  useEffect(() => {
      ...
      if (isConfigOpen && featureType === 'REVOLVE' && activeAxisId) {
          ...
          if (p1 && p2) {
              ...
              const center = new THREE.Vector3((p1.x + p2.x)/2, (p1.y + p2.y)/2, 0);
              ...
              const material = new THREE.LineDashedMaterial({ color: 0xffa500, linewidth: 2, scale: 1, dashSize: 10, gapSize: 5 });
  ...
  useEffect(() => {
    ...
    if (selectedFaceData) {
        ...
        geometry.setAttribute('position', new THREE.BufferAttribute(selectedFaceData.faceVertices, 3));
        const mesh = new THREE.Mesh(geometry, new THREE.MeshBasicMaterial({ color: 0xffaa00, side: THREE.DoubleSide, transparent: true, opacity: 0.25, depthTest: false }));
  ...
  const extractFace = (mesh: THREE.Mesh, faceIndex: number) => {
    ...
    const vA = getV(i1), vB = getV(i2), vC = getV(i3);
    const n = new THREE.Vector3().crossVectors(new THREE.Vector3().subVectors(vB, vA), new THREE.Vector3().subVectors(vC, vA));
    if (n.lengthSq() < 1e-12) return { vertices: new Float32Array([]), boundary: [], normal: new THREE.Vector3(0,0,1) };
    n.normalize();
    ...
    const edgeKey = (va: THREE.Vector3, vb: THREE.Vector3) => { const ka = key(va), kb = key(vb); return ka < kb ? `${ka}|${kb}` : `${kb}|${ka}`; };
    ...
    for(let f=0; f<count; f++) {
        ...
        const va = getV(idx1), vb = getV(idx2), vc = getV(idx3);
        const fn = new THREE.Vector3().crossVectors(new THREE.Vector3().subVectors(vb, va), new THREE.Vector3().subVectors(vc, va)).normalize();
        if (fn.dot(n) < 0.9) continue;
        vertices.push(va.x, va.y, va.z, vb.x, vb.y, vb.z, vc.x, vc.y, vc.z);
        [[va, vb], [vb, vc], [vc, va]].forEach(pair => {
    ...
    edges.forEach(e => { if (e.count === 1) boundary.push(new THREE.Line3(e.a, e.b)); });
    return { vertices: new Float32Array(vertices), boundary, normal: n };
  };

  const onCanvasMouseMove = (event: React.MouseEvent) => {
      if (isConfigOpen || !sketchVisGroupRef.current || !cameraRef.current) return;
      ...
      const intersects = raycasterRef.current.intersectObjects(sketchVisGroupRef.current.children, true);
  ...
  const onCanvasClick = (event: React.MouseEvent) => {
      if (!meshGroupRef.current || !historyGroupRef.current || !sketchVisGroupRef.current || !cameraRef.current) return;
      ...
      const sketchIntersects = raycasterRef.current.intersectObjects(sketchVisGroupRef.current.children, true);
      ...
      if (sketchIntersects.length > 0) {
          ...
          if (sketchId) {
              setSelectedSketchElements(prev => {
                  if (prev.includes(sketchId)) return prev.filter(id => id !== sketchId);
      ...
      if (intersects.length > 0) {
          ...
          if (hit.face) {
              ...
              if (faceData.vertices.length > 0) {
                  const normalWorld = faceData.normal.clone().transformDirection(mesh.matrixWorld).normalize();
                  setSelectedFaceData({
                      point: hit.point,
                      normal: normalWorld,
  ...
  const handleCommit = () => {
      if (!errorMsg) {
          onCommitExtrusion(localDepth, operation, throughAll, featureType, revolveAngle, activeAxisId || undefined);
  ...
  const handleCreateSketchOnFace = () => {
      ...
      if (boundaryEdges.length === 0) { alert("Could not detect face boundary."); return; }
      ...
      let xAxis = new THREE.Vector3().subVectors(endWorld, originWorld).normalize();
      if (Math.abs(xAxis.dot(zAxis)) > 0.9) {
          xAxis = new THREE.Vector3().crossVectors(zAxis, new THREE.Vector3(0,1,0)).normalize();
          if (xAxis.lengthSq() < 0.1) xAxis = new THREE.Vector3().crossVectors(zAxis, new THREE.Vector3(1,0,0)).normalize();
      }
      ...
      const findPoint = (x: number, y: number) => projectedPoints.find(p => Math.abs(p.x - x) < 0.001 && Math.abs(p.y - y) < 0.001);
      boundaryEdges.forEach(edge => {
          ...
          if (!p1) { p1 = { id: `p_proj_${Math.random().toString(36).substr(2, 9)}`, x: startSketch.x, y: startSketch.y, fixed: true }; projectedPoints.push(p1); }
          let p2 = findPoint(endSketch.x, endSketch.y);
          if (!p2) { p2 = { id: `p_proj_${Math.random().toString(36).substr(2, 9)}`, x: endSketch.x, y: endSketch.y, fixed: true }; projectedPoints.push(p2); }
          projectedLines.push({ id: `l_proj_${Math.random().toString(36).substr(2, 9)}`, p1: p1.id, p2: p2.id, construction: true });
      });
      onSketchOnFace(projectedLines, projectedPoints, localToWorld.toArray(), [], []);
  };
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-4 right-4 z-10 flex gap-2">
            <button onClick={fitView} className="bg-[#1a1a1a]/90 border border-white/10 text-white px-3 py-2 rounded-xl text-xs font-bold uppercase hover:bg-white/10" title="Recenter View">
               ⛶
            </button>
            <button onClick={handleExportSTL} className="bg-[#1a1a1a]/90 border border-white/10 text-white px-4 py-2 rounded-xl text-xs font-bold uppercase hover:bg-white/10 flex items-center gap-2">
      ...
      {!isConfigOpen && hasActiveSketch && (
         <div className="absolute top-4 left-4 z-10 flex flex-col gap-2 animate-in fade-in">
             ...
             <div className="flex flex-col gap-1 mt-2">
                <span className="text-[10px] font-bold text-gray-500 uppercase tracking-widest px-2">Operations</span>
                <button 
                    onClick={() => { setFeatureType('EXTRUDE'); setIsConfigOpen(true); }}
                ...
                >
                    ...
                    <div className="flex flex-col">
                        ...
                        <span className="text-[9px] opacity-80">
                            {selectedSketchElements.length > 0 ? `${selectedSketchElements.length} selected items` : 'Pull entire sketch'}
                        </span>
                    </div>
                </button>
                <button 
                    onClick={() => { setFeatureType('REVOLVE'); setIsConfigOpen(true); }}
                ...
                >
                    ...
                    <div className="flex flex-col">
                        ...
                        <span className="text-[9px] opacity-80">
                            {selectedSketchElements.length > 0 ? `${selectedSketchElements.length} selected items` : 'Spin entire sketch'}
                        </span>
                    </div>
                </button>
             </div>
      ...
      {isConfigOpen && (
        <div className="absolute top-4 left-4 z-10 bg-[#1a1a1a]/95 backdrop-blur-xl p-5 rounded-2xl border border-white/10 shadow-2xl w-72 flex flex-col gap-4 animate-in slide-in-from-left-4" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between border-b border-white/5 pb-3">
                <h3 className="text-xs font-bold text-white uppercase tracking-wider flex items-center gap-2">
                    <span className="text-lg">⚙️</span> {initialFeatureParams ? 'Edit Feature' : 'Feature Properties'}
                </h3>
            </div>
            ...
            <div className="flex bg-[#000] p-1 rounded-lg mb-2">
                <button 
                    ...
                    className={`flex-1 py-1.5 text-[10px] font-bold uppercase rounded-md transition-colors ${featureType === 'EXTRUDE' ? 'bg-purple-600 text-white' : 'text-gray-500 hover:text-gray-300'}`}
                >
                    Extrude
                </button>
                <button 
                    onClick={() => setFeatureType('REVOLVE')} 
                    className={`flex-1 py-1.5 text-[10px] font-bold uppercase rounded-md transition-colors ${featureType === 'REVOLVE' ? 'bg-orange-600 text-white' : 'text-gray-500 hover:text-gray-300'}`}
                >
                    Revolve
                </button>
            </div>
            ...
            <div className="flex bg-[#000] p-1 rounded-lg">
                <button 
                    ...
                    className={`flex-1 py-1.5 text-[10px] font-bold uppercase rounded-md transition-colors ${operation === 'NEW' ? 'bg-blue-600 text-white' : 'text-gray-500 hover:text-gray-300'}`}
                >
                    New
                </button>
                <button 
                    onClick={() => setOperation('CUT')} 
                    className={`flex-1 py-1.5 text-[10px] font-bold uppercase rounded-md transition-colors ${operation === 'CUT' ? 'bg-red-600 text-white' : 'text-gray-500 hover:text-gray-300'}`}
                >
                    Cut
                </button>
            </div>
            ...
            {featureType === 'EXTRUDE' && (
                ...
                <div className="flex items-center gap-2">
                     ...
                     />
                     <label htmlFor="throughAll" className="text-xs text-gray-300 font-medium select-none">Through All</label>
                </div>
                <div className={`space-y-2 transition-opacity ${throughAll ? 'opacity-40 pointer-events-none' : 'opacity-100'}`}>
                    <label className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">Depth (mm)</label>
                    <div className="flex items-center gap-3">
                        <div className="relative flex-1">
                            <input type="number" min="1" max="1000" value={localDepth} onChange={(e) => setLocalDepth(parseFloat(e.target.value))} className="w-full bg-[#0a0a0a] border border-white/10 rounded-lg py-2 px-3 text-sm font-mono text-blue-400 focus:outline-none focus:border-blue-500" />
                            <span className="absolute right-3 top-2 text-xs text-gray-600 pointer-events-none">mm</span>
                        </div>
                    </div>
                    <input type="range" min="1" max="500" value={localDepth} onChange={(e) => setLocalDepth(parseFloat(e.target.value))} className="w-full h-1 bg-gray-700 rounded-lg appearance-none cursor-pointer" />
                </div>
                </>
            )}
            ...
            {featureType === 'REVOLVE' && (
                <>
                 <div className="space-y-2">
                    <label className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">Angle (Deg)</label>
                    <div className="flex items-center gap-3">
                        <input type="number" min="1" max="360" value={revolveAngle} onChange={(e) => setRevolveAngle(parseFloat(e.target.value))} className="w-full bg-[#0a0a0a] border border-white/10 rounded-lg py-2 px-3 text-sm font-mono text-orange-400 focus:outline-none focus:border-orange-500" />
                    </div>
                    <input type="range" min="1" max="360" value={revolveAngle} onChange={(e) => setRevolveAngle(parseFloat(e.target.value))} className="w-full h-1 bg-gray-700 rounded-lg appearance-none cursor-pointer" />
                 </div>
                 <div className="space-y-1">
                    <label className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">Axis Line</label>
                    <select 
                        value={activeAxisId || ''} 
                    ...
                    >
                        ...
                        {state.lines.map((l, i) => (
                             <option key={l.id} value={l.id}>
                                Line {i+1} {l.construction ? '(Constr)' : ''} {l.id === activeAxisId ? '(Selected)' : ''}
                             </option>
                        ))}
                    </select>
                    {!activeAxisId && <p className="text-[10px] text-red-400">Please select a line to revolve around.</p>}
                 </div>
                </>
            )}
            ...
            {initialFeatureParams && (
                <div className="pt-2 border-t border-white/5">
                    <button onClick={handleEditSketch} className="w-full py-2 mb-2 rounded-lg bg-[#2a2a2a] hover:bg-[#333] border border-white/5 text-xs font-bold text-purple-400 uppercase flex items-center justify-center gap-2">
                       <span>✎</span> Edit Sketch Geometry
                    </button>
                </div>
            )}

            {errorMsg && <div className="p-3 bg-red-500/10 border border-red-500/20 rounded-lg"><p className="text-[10px] text-red-400 font-bold">{errorMsg}</p></div>}

            <div className="flex gap-2 pt-2">
                 <button onClick={() => setIsConfigOpen(false)} className="flex-1 py-2 rounded-lg bg-[#333] hover:bg-[#444] text-xs font-bold text-gray-300 uppercase">Cancel</button>
                 <button onClick={handleCommit} disabled={!!errorMsg} className={`flex-1 py-2 rounded-lg text-xs font-bold text-white uppercase shadow-lg ${errorMsg ? 'bg-gray-700' : 'bg-blue-600 hover:bg-blue-500'}`}>OK</button>
            </div>
        </div>
      )}

      {selectedFaceData && !isConfigOpen && (
          <div className="absolute z-20 bg-black/80 backdrop-blur border border-blue-500/50 text-white p-2 rounded-lg shadow-lg pointer-events-none" style={{ left: '50%', top: '50%', transform: 'translate(-50%, -50%)' }}>
              <div className="text-xs font-bold mb-1 text-blue-300 text-center">Face Selected</div>
              <button onClick={(e) => { e.stopPropagation(); handleCreateSketchOnFace(); }} className="pointer-events-auto bg-blue-600 hover:bg-blue-500 text-white text-[10px] font-bold uppercase px-3 py-1.5 rounded shadow-lg">Sketch on Face</button>
          </div>
      )}
    </div>
  );
